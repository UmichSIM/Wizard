...
class DualControl(object):
    def __init__(self, world, start_in_autopilot):
        ...
        self._state = 0
        # auto transfer of control
        self._driver_change = False
    ...
    def parse_events(self, world, clock):
        ...
        if not self._autopilot_enabled:
            if isinstance(self._control, carla.VehicleControl):
                ...
                # Wizard
                if self._driver != world._driver:
                    # set controller driver to be world driver
                    # self._driver = world._driver
                    self._driver_change = True               
                self._parse_vehicle_wheel()
                # update world driver if key input for change by controllers
                world._driver = self._driver
    ...
    def _parse_vehicle_wheel(self):
        ...
        if self._driver_change:
            if self._driver == 'Wizard':
                self._joystick = self._joystickD
                forceOff(self.wheelTwo)
                #forceFollow(self.wheelTwo, self._joystickW.get_axis(0))
                self._state = 0
            elif self._driver == 'Human':
